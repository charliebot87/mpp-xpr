import { decodeFunctionData, encodeFunctionData, getAbiItem, isAddressEqual, toFunctionSelector, } from 'viem';
 ...
 */
export async function settleOnChain(client, escrowContract, voucher, feePayer) {
 ...
 */
export async function closeOnChain(client, escrowContract, voucher, account, feePayer) {
    assertUint128(voucher.cumulativeAmount);
 ...
 */
async function sendFeePayerTx(client, feePayer, to, data, label) {
...
export async function broadcastOpenTransaction(parameters) {
    ...
    const [payee, token, deposit, , authorizedSigner] = openArgs;
