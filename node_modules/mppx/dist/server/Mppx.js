...
export function create(config) {
    const { realm = Env.get('realm') ?? 'MPP Payment', secretKey = Env.get('secretKey'), transport = Transport.http(), } = config;
    if (!secretKey) {
...
function createMethodFn(parameters) {
    ...
    return (options) => {
        ...
        return Object.assign(async (input) => {
            ...
            {
                const routeReq = challenge.request;
                const echoedReq = credential.challenge.request;
                for (const field of ['amount', 'currency', 'recipient']) {
                    if (routeReq[field] !== undefined &&
        ...
        if (paymentHeader) {
            ...
            if (credential) {
                ...
                const candidates = handlers.filter((h) => {
                    ...
                    if (!canonical)
                        return true;
                    for (const field of ['amount', 'currency', 'recipient', 'chainId']) {
